...
from data.Keyboards.Reply.to_menu import return_keyboard

from loader import storage, cleaner, session
from loader import dp, bot, notificator
import requests
import functools
...
async def mark_user(query: types.CallbackQuery, state: FSMContext):
...
async def mark_all_users(query: types.CallbackQuery, state: FSMContext):
    previous_data = query.message.reply_markup['inline_keyboard']
    new_data = [[data[0].text, data[0].callback_data] for data in previous_data]
